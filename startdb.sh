...
if [[ "$(docker ps -a | grep poll_database)" == "" ]]; then
    docker run --rm --name poll_database -d -p 27017:27017  mongo
    echo Mongo container[poll_database] started at port 27017
else
    echo Mongo container[poll_database] already running at port 27017
fi